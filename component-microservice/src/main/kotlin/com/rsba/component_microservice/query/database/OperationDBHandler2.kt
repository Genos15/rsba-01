...
object OperationDBHandler2 {
    ...
    fun all(row: Row?): List<Operation> = try {
        if (row != null) {
    ...
    fun one(row: Row?): Optional<Operation> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<Operation>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
